...
CREATE PROCEDURE [dbo].[sp_stg_trns_02](
@fechaini date, @fechafin date)
AS
BEGIN

PRINT(concat('inicia transaccion 02 ',SYSDATETIME()));
...
FROM [10.110.21.9].[LIBRERIA].DBO.FACTURASVENTA F 
    LEFT JOIN [10.110.21.9].[LIBRERIA].DBO.ALBVENTACAB AL             WITH (NOLOCK)
        ON (F.NUMSERIE=AL.NUMSERIEFAC AND F.NUMFACTURA=AL.NUMFAC AND F.N=AL.NFAC)
    LEFT JOIN [10.110.21.9].[LIBRERIA].DBO.ALBVENTALIN L              WITH (NOLOCK)
        ON (AL.NUMSERIE=L.NUMSERIE AND AL.NUMALBARAN=L.NUMALBARAN AND AL.N=L.N)
    LEFT JOIN [10.110.21.9].[LIBRERIA].DBO.CLIENTES C                 WITH (NOLOCK)
        ON C.CODCLIENTE = F.CODCLIENTE
    LEFT JOIN [10.110.21.9].[LIBRERIA].DBO.ARTICULOS AR               WITH (NOLOCK)
        ON (L.CODARTICULO=AR.CODARTICULO)
    LEFT JOIN [10.110.21.9].[LIBRERIA].DBO.SERIESCAMPOSLIBRES SL      WITH (NOLOCK)
        ON (SL.SERIE=F.CAJA)
    LEFT JOIN [10.110.21.9].[LIBRERIA].DBO.IMPUESTOS I                WITH (NOLOCK)

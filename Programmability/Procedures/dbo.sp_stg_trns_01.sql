...
CREATE PROCEDURE [dbo].[sp_stg_trns_01](
@fechaini date, @fechafin date)
AS
BEGIN

PRINT(concat('inicia transaccion 01 ',SYSDATETIME()));
...
FROM [10.110.21.9].[PANISTERIA].DBO.FACTURASVENTA F 
    LEFT JOIN [10.110.21.9].[PANISTERIA].DBO.ALBVENTACAB AL             WITH (NOLOCK)
        ON (F.NUMSERIE=AL.NUMSERIEFAC AND F.NUMFACTURA=AL.NUMFAC AND F.N=AL.NFAC)
    LEFT JOIN [10.110.21.9].[PANISTERIA].DBO.ALBVENTALIN L              WITH (NOLOCK)
        ON (AL.NUMSERIE=L.NUMSERIE AND AL.NUMALBARAN=L.NUMALBARAN AND AL.N=L.N)
    LEFT JOIN [10.110.21.9].[PANISTERIA].DBO.CLIENTES C                 WITH (NOLOCK)
        ON C.CODCLIENTE = F.CODCLIENTE
    LEFT JOIN [10.110.21.9].[PANISTERIA].DBO.ARTICULOS AR               WITH (NOLOCK)
        ON (L.CODARTICULO=AR.CODARTICULO)
    LEFT JOIN [10.110.21.9].[PANISTERIA].DBO.SERIESCAMPOSLIBRES SL      WITH (NOLOCK)
        ON (SL.SERIE=F.CAJA)
    LEFT JOIN [10.110.21.9].[PANISTERIA].DBO.IMPUESTOS I                WITH (NOLOCK)
